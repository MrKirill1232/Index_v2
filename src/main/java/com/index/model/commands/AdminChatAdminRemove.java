...
public class AdminChatAdminRemove
{
    public AdminChatAdminRemove(Update update)
    {
        ...
        chat.removeFromAdminList(replyUser.getUserID());
        ChatInfo.getInstance().storeMe(chatID);
        StringBuilder message = new StringBuilder(getClass().getSimpleName());
        if (chat.getAdminsList().contains(replyUser.getUserID()))
        {
            message.append(": у пользователя ").append(replyUser.getUserName()).append(" все еще остаются права администратора;");
        }
        else if (chat.getUserModeration().contains(replyUser.getUserID()))
        {
            message.append(": у пользователя ").append(replyUser.getUserName()).append(" все еще остаются права пользовательской модерации;");
        }
        else
        {
            message.append(": у пользователя ").append(replyUser.getUserName()).append(" более нет прав администратора;");
        }
